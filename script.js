...
const SPEED_2 = 1.0 / 60.0 / 1.5;

const realWidth = window.innerWidth + 2 * GRID_SIZE;
const realHeight = window.innerHeight + 2 * GRID_SIZE;

const RESOLUTION = realHeight / GRID_SIZE;
...
renderEl.style.fontSize = GRID_SIZE + "px";
...
class FlipFluid {
  ...
  handleParticleCollisions(obstacleX, obstacleY, obstacleRadius) {
    ...
    for (var i = 0; i < this.numParticles; i++) {
      ...
      const minXLocal = 0;
      const maxXLocal = simWidth - minXLocal;
...
function update() {
  const MAX_RADIUS = window.innerWidth > window.innerHeight ? 0.5 : 0.4;
  scene.obstacleRadius = (scene.obstacleRadius * 3 + MAX_RADIUS) / 4;
  simulate();

  let toRender = "";

  for (let i = f.fNumY - 2; i > 0; i--) {
    let row = "";
